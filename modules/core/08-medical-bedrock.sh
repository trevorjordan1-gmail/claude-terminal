...
if [ "$(sudo cat "$MANAGED" 2>/dev/null)" != "$NEW" ]; then
    # (Re)pinning: this is the moment a typo would become a dead terminal, and
    # the only time the daily run should spend a network call on it.
    # Never fail on the listing itself (no aws, no permission, no network at
    # provision time are all fine).
    if have aws; then
